...
/**
 * AlmacenGUI - Versión gráfica del CRUD de productos. Utiliza Swing para la
 * interfaz y se apoya en la interfaz ModeloAbs.
 */
public class AlmacenGUI extends JFrame {
	...
	private void actualizarTabla(boolean soloStockBajo) {
		...
		// Uso del método supuesto obtenerTodos() o listarProductosStockMin()
		List<Producto> lista = soloStockBajo ? almacen.listarProductosStockMin() : almacen.obtenerTodos();

		for (Producto p : lista) {
			Object[] fila = { p.getCodigo(), p.getNombre(), p.getStock(), p.getStock_min(),
	...
	private void modificarPrecio() {
		...
		try {
			float nuevoPrecio = Float.parseFloat(
					JOptionPane.showInputDialog(this, "Nuevo precio para " + p.getNombre() + ":", p.getPrecio()));
			if (nuevoPrecio > 0) {
	...
	private void transaccionStock(boolean esCompra) {
		...
		try {
			...
			int cantidad = Integer
					.parseInt(JOptionPane.showInputDialog(this, "Cantidad:", titulo, JOptionPane.QUESTION_MESSAGE));
			if (cantidad <= 0) {
	...
	private void cargarDatosPrueba() {
		...
		p1.setStock(15);
		p1.setStock_min(5);
		p1.setPrecio(159.99f);

		Producto p2 = new Producto(102, "Teclado Mecánico");
		p2.setStock(4);
		p2.setStock_min(10);
		p2.setPrecio(45.50f); // Este saldrá en "Stock Mínimo"
		...
		p4.setStock(2);
		p4.setStock_min(5);
		p4.setPrecio(89.90f); // Este también bajo mínimos
